...
export default function Room({ theme: t, bgmOn, setBgmOn, mode, onEnterPrivate, onEnterNuonuo, onEnterBedroom, onEnterKitchen }) {
  ...
  const contentMap = {
    clock: (
      <div style={{ textAlign:"center", padding:"24px 24px 32px", fontFamily:"'Noto Serif SC',serif" }}>
        <img src="/family_portrait.jpg" alt="" style={{ width:"clamp(180px,60vw,280px)", marginBottom:20, borderRadius:10, opacity:.95 }} />
  ...
  return (
    <div style={{ position:"fixed", inset:0, overflow:"hidden" }}>
      ...
      {/* 游戏模式：梦境背景图（厨房） */}
      <img
        src="/dream-kitchen.jpg"
        alt=""
        style={{
          position:"absolute", top:0, left:0, width:"100%", height:"auto",
          zIndex:2, pointerEvents:"none",
          opacity: gameMode && active === "kitchen" ? 1 : 0,
          transition: "opacity 1.5s ease",
        }}
      />
      {/* 游戏模式：梦境背景图（糯糯房间） */}
      ...
      {/* 左上 logo */}
      <div style={{ position:"absolute", top:14, left:16, zIndex:10, fontSize:11, color:t.text, opacity:.38, fontFamily:"'Noto Serif SC',serif", letterSpacing:".1em" }}>DuLee</div>
      {/* 右下 日夜 */}
