...
const CW = 280; const CH = 210;
const CLAW_Y0 = 26;
const DROP_Y  = 162;
const BOUNCE_SPEED = 88;
...
function drawClaw(ctx, x, y, open) {
  ...
  ctx.lineWidth = 2;
  offsets.forEach(ox => {
    ctx.beginPath(); ctx.moveTo(x, y - 2); ctx.lineTo(x + ox, y + 16); ctx.stroke();
    ctx.beginPath(); ctx.arc(x + ox, y + 16, 3, 0, Math.PI, ox >= 0); ctx.stroke();
  });
}
...
function CraneRecordsList({ records, t }) {
      ...
      {records.map((r, i) => (
        <div key={i} style={{
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
            <span style={{ color: r.result === "win" ? "#C8A040" : t.textMuted }}>
              {r.result === "win" ? "✦ " : "✗ "}
            ...
            {r.wallet === "private" && (
              <span style={{ fontSize:10, color:"rgba(200,160,60,0.5)" }}>私密币投入</span>
            )}
          </div>
...
export default function CraneGame({ theme: t }) {
  ...
  const g = useRef({
    ...
    open: 0, grabT: 0, won: false, wonToy: null,
  ...
  const fetchBalance = useCallback(async () => {
    try {
      ...
      if (authRes.ok) {
        ...
        setIsLee(false); setPrivBal(null);
      }
    } catch { setPubBal(0); }
  }, []);
  ...
  async function loadRecords() {
    const d = await fetch("/api/farm/game-records?game=crane&limit=30").then(r => r.json()).catch(() => ({ records: [] }));
    setRecords(d.records || []);
  }

  // 写游戏记录（phase 变 done 时触发）
  useEffect(() => {
    if (phase === "done" && gameResultRef.current) {
      const { won, wonToy } = gameResultRef.current;
      const who = isLeeRef.current ? "黎" : (playerIdRef.current || "游客");
      fetch("/api/farm/game-records", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({
          game: "crane", who,
          wallet: isLeeRef.current ? walletRef.current : null,
          result: won ? "win" : "lose",
          detail: won ? `夹到${wonToy.emoji} ${wonToy.name}` : "没夹到",
        }),
      }).catch(() => {});
    ...
    function loop(ts) {
      ...
      if (s.phase === "grabbing") {
        ...
        if (p < 0.5)     s.open = p * 2;
          ...
          gameResultRef.current = { won: s.won, wonToy: s.wonToy };
          s.phase = "rising";
          if (s.won) { setWonToyUI(s.wonToy); addPendingToy(s.wonToy, s.playerId); setPending(getPendingToys()); }
        }
      }
      if (s.phase === "rising") {
        s.y -= RISE_SPEED * dt;
        if (s.y <= CLAW_Y0) {
          s.y = CLAW_Y0; s.phase = "done"; setPhase("done");
          setTimeout(() => { s.phase = "idle"; s.wonToy = null; setPhase("idle"); setWonToyUI(null); }, 2200);
        }
      }

      ctx.clearRect(0, 0, CW, CH);
      ctx.fillStyle = "#110C05"; ctx.fillRect(0, 0, CW, CH);
      const grd = ctx.createRadialGradient(s.x, 0, 0, s.x, 20, 110);
      grd.addColorStop(0, "rgba(255,190,60,0.10)"); grd.addColorStop(1, "rgba(0,0,0,0)");
      ...
      ctx.font = "20px serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.globalAlpha = 0.82;
      s.toyArrange.forEach((toy, i) => ctx.fillText(toy.emoji, TOY_POS[i].x, TOY_POS[i].y));
      ctx.globalAlpha = 1;

      drawClaw(ctx, s.x, s.y, s.open);

      if (s.won && (s.phase === "rising" || s.phase === "done") && s.wonToy) {
        ctx.font = "18px serif"; ctx.fillText(s.wonToy.emoji, s.x, s.y + 22);
      }
  ...
  async function insertCoin() {
    ...
    g.current.x = CW / 2; g.current.vx = BOUNCE_SPEED * (Math.random() > 0.5 ? 1 : -1);
    setPhase("running"); fetchBalance();
  }

  function drop() {
    if (g.current.phase !== "running") return;
    g.current.phase = "dropping"; g.current.vx = 0; setPhaseUI("dropping");
  }

  const isIdle = phase === "idle"; const isRunning = phase === "running";
  const canAfford = balance !== null && balance >= 5;
  let resultMsg = "";
  if (phase === "done") resultMsg = wonToyUI ? `夹到啦！${wonToyUI.emoji} ${wonToyUI.name}` : "没夹到呢 🥺";

  return (
    <div style={{ fontFamily:"'Noto Serif SC',serif", userSelect:"none", maxWidth:320, margin:"0 auto" }}>
      <GameTabs t={t} tab={tab} setTab={setTab} onRecords={loadRecords} />

      {tab === "game" ? (
        <>
          {/* 玩家 ID + 钱包 */}
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
            <input value={playerId} onChange={e => setPlayerId(e.target.value.slice(0,8))}
              placeholder="留个名字（可不填）" maxLength={8}
              style={{ flex:1, padding:"6px 12px", borderRadius:8, border:`1px solid ${t.surfaceBorder}`,
                background:"rgba(20,14,6,0.6)", color:t.text, fontSize:12, outline:"none", fontFamily:"inherit" }} />
            <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:2 }}>
              {isLee ? (
                <>
                  ...
                  <span style={{ fontSize:10, color:t.textMuted }}>🪙 {wallet === "public" ? (pubBal ?? "…") : (privBal ?? "…")}</span>
                </>
              ) : (
                <span style={{ fontSize:11, color:t.textMuted, whiteSpace:"nowrap" }}>🪙 {pubBal ?? "…"}</span>
              )}
              <button onClick={fetchBalance} style={{ background:"none", border:"none", cursor:"pointer", fontSize:11, color:t.textMuted, padding:"0 2px", lineHeight:1 }}>↻</button>
            </div>
          </div>

          {/* 机器外壳 */}
          <div style={{ background:"#2A1E0F", borderRadius:16, overflow:"hidden",
            boxShadow:"0 6px 28px rgba(0,0,0,0.6), inset 0 1px 0 rgba(200,160,60,0.2)" }}>
            <div style={{ padding:"7px 16px", background:"rgba(200,160,60,0.10)",
              borderBottom:"1px solid rgba(200,160,60,0.2)", textAlign:"center",
              fontSize:11, color:"rgba(200,160,60,0.8)", letterSpacing:"0.15em" }}>
              ✦ 夹娃娃机 ✦
            </div>
            <div style={{ margin:"10px 10px 0", borderRadius:8, border:"1.5px solid rgba(200,160,60,0.35)",
              overflow:"hidden", position:"relative", background:"#110C05", boxShadow:"inset 0 0 20px rgba(0,0,0,0.5)" }}>
              <canvas ref={canvasRef} width={CW} height={CH} style={{ display:"block", width:"100%", height:"auto" }} />
              {phase === "done" && (
                <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center",
                  background:"rgba(10,7,3,0.55)", backdropFilter:"blur(2px)" }}>
                  <div style={{ textAlign:"center", padding:"14px 22px", borderRadius:14,
                    background:"rgba(20,14,6,0.92)",
                    border:`1px solid ${wonToyUI ? "rgba(200,160,60,0.6)" : "rgba(120,90,50,0.35)"}` }}>
                    <div style={{ fontSize:wonToyUI ? 32 : 22, marginBottom:4 }}>{wonToyUI ? wonToyUI.emoji : "🤲"}</div>
                    <div style={{ fontSize:13, color: wonToyUI ? "#C8A040" : "rgba(160,130,70,0.7)", fontWeight:600 }}>{resultMsg}</div>
                    {wonToyUI && <div style={{ fontSize:10, color:"rgba(160,130,60,0.6)", marginTop:5, lineHeight:1.6 }}>去客厅找糯糯送给她 →</div>}
                  </div>
                </div>
              )}
            </div>
            <div style={{ padding:"12px 14px 14px", background:"rgba(0,0,0,0.25)",
              display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
              {isIdle && (
                <button onClick={insertCoin} disabled={!canAfford} style={{
              ...
              {isRunning && (
                <button onClick={drop} style={{
                  padding:"10px 32px", borderRadius:10, fontSize:14,
                  border:"1.5px solid rgba(180,220,120,0.6)", background:"rgba(140,200,80,0.15)",
                  color:"rgba(180,230,120,0.9)", fontFamily:"inherit", fontWeight:700, cursor:"pointer",
            ...
            <div style={{ padding:"4px 0 8px", textAlign:"center", fontSize:10, color:"rgba(160,120,50,0.35)", letterSpacing:"0.1em" }}>
              ▭  COIN  ▭
            </div>
          </div>
          ...
          {/* 暂存区 */}
          {pending.length > 0 && (
            <div style={{ marginTop:14, padding:"12px 14px", background:"rgba(200,160,60,0.06)",
              border:"1px solid rgba(200,160,60,0.2)", borderRadius:12 }}>
              <div style={{ fontSize:11, color:"rgba(200,160,60,0.7)", marginBottom:8, fontWeight:600 }}>🎁 暂存区（离开小屋后清空）</div>
              {pending.map((toy, i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:"4px 0", fontSize:12, color:t.textSub }}>
                  ...
                  <span>{toy.name}</span>
                  <span style={{ fontSize:10, color:t.textMuted, marginLeft:"auto" }}>{toy.playerId}</span>
                </div>
              ))}
              <div style={{ marginTop:8, fontSize:10, color:"rgba(160,130,60,0.55)", lineHeight:1.7 }}>去客厅找糯糯，点她就能送出去 🐾</div>
            </div>
          )}
        </>
      ) : (
        <CraneRecordsList records={records} t={t} />
      )}
